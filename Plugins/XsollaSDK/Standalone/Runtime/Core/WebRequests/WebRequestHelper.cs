...
{
	internal partial class WebRequestHelper : MonoBehaviour
	{
		...
		private IEnumerator PerformWebRequest<T>(UnityWebRequest webRequest, Action<T, Dictionary<string, string>> onComplete, Action<Error> onError, ErrorGroup errorsToCheck) where T : class
		{
			yield return InternalPerformWebRequest(
				webRequest,
				() => ProcessRequest(webRequest, onComplete, onError, errorsToCheck));
