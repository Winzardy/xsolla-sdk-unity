...
namespace Xsolla.Core
{
	internal partial class WebRequestHelper
	{
		...
		public void GetRequest<T>(SdkType sdkType, string url, WebRequestHeader requestHeader, Action<T, Dictionary<string, string>> onComplete, Action<Error> onError, ErrorGroup errorsToCheck = ErrorGroup.CommonErrors) where T : class
		{
			var headers = AppendAnalyticHeaders(sdkType, requestHeader);
			StartCoroutine(PerformGet(sdkType, url, onComplete, onError, headers, errorsToCheck));
		}
    ...
    public void GetRequest<T>(SdkType sdkType, string url, Action<T> onComplete, Action<Error> onError, ErrorGroup errorsToCheck = ErrorGroup.CommonErrors) where T : class
		...
		private IEnumerator PerformGet<T>(SdkType sdkType, string url, Action<T, Dictionary<string, string>> onComplete, Action<Error> onError, List<WebRequestHeader> requestHeaders, ErrorGroup errorsToCheck = ErrorGroup.CommonErrors) where T : class
		{
			url = AppendAnalyticsToUrl(sdkType, url);
