...
namespace Xsolla.Core
{
	internal partial class WebRequestHelper
	{
		...
		{
			var sdkTypeValue = Info.SDK_NAME;
			if (string.IsNullOrEmpty(Info.SDK_NAME))
				sdkTypeValue = SdkTypeConverter.GetSdkTypeAsString(sdkType);
		...
		{
			var sdkVersion = Info.SDK_VERSION;
		...
		{
			var buildPlatform = Application.isEditor 
				? (EditorProvider.Handler != null ? EditorProvider.Handler.GetActiveBuildTargetAsString() : "editor")
				: RuntimePlatformConverter.GetRunningPlatformAsString();
