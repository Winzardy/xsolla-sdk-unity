...
{
	[Serializable]
	internal class RedirectPolicySettings
	{
		...
		public static RedirectPolicy GeneratePolicy(XsollaSettings settings)
		{
			if (Application.platform == RuntimePlatform.WebGLPlayer)
			{
				return !settings.WebglRedirectPolicySettings.UseSettingsFromPublisherAccount
					? settings.WebglRedirectPolicySettings.CreatePolicy()
					: null;
			}

			return !settings.DesktopRedirectPolicySettings.UseSettingsFromPublisherAccount
				? settings.DesktopRedirectPolicySettings.CreatePolicy()
				: null;
		}
