using UnityEngine;
using Xsolla.Core;
using DeviceType = Xsolla.Core.DeviceType;
...
namespace Xsolla.Auth
{
	internal class DeviceInfo
	{
		...
		public static DeviceInfo Create()
		{
			...
			switch (DataProvider.GetPlatform())
			{
				case RuntimePlatform.WindowsPlayer:
				case RuntimePlatform.WindowsEditor:
					deviceInfo.DeviceType = DeviceType.Windows;
					break;
				case RuntimePlatform.OSXPlayer:
				case RuntimePlatform.OSXEditor:
					deviceInfo.DeviceType = DeviceType.MacOS;
					break;
				case RuntimePlatform.WebGLPlayer:
					deviceInfo.DeviceType = DeviceType.Web;
					break;
				case RuntimePlatform.Android:
					deviceInfo.DeviceType = DeviceType.Android;
					break;
				case RuntimePlatform.IPhonePlayer:
					deviceInfo.DeviceType = DeviceType.iOS;
					break;
				default:
					deviceInfo.DeviceType = DeviceType.Windows;
					break;
	...
	internal static class DeviceInfoExtensions
	{
		...
		public static string GetSafeDeviceId(this DeviceInfo deviceInfo)
		{
			...
			if (deviceInfo.DeviceType == Core.DeviceType.Windows)
			{
				...
				{
					XDebug.LogWarning(
						$"Device ID is too short. It will be padded to {minDeviceIdLength} symbols. Original device ID: {deviceId}");
					deviceId = deviceId.PadLeft(minDeviceIdLength, '0');
				}
				else if (deviceId.Length > maxDeviceIdLength)
				{
					XDebug.LogWarning(
						$"Device ID is too long. It will be truncated to {maxDeviceIdLength} symbols. Original device ID: {deviceId}");
