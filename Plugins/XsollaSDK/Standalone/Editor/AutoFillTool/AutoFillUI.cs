...
{
	internal class AutoFillUI : EditorWindow
	{
		...
		public event Action<int[], GotoType> OnGoToPA;
		...
		private void OnGUI()
		{
			...
			if (GUILayout.Button("Xsolla Login Settings",_buttonStyle)) {GoToPA(GotoType.Login);}
			if (GUILayout.Button("Xsolla IAP Catalog",_buttonStyle)) {GoToPA(GotoType.Items);}
			GuiLockEnd();
		...
		private void GoToPA(GotoType type)
		{
			if (OnGoToPA != null)
				OnGoToPA.Invoke(_selectionCur, type);
			else
				ShowMessage($"DEBUG: Go to PA [{string.Join(",",_selectionCur)}]");
		}
