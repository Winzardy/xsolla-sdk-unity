...
namespace Xsolla.Core.Editor.AutoFillSettings
{
	...
	internal class AutoFillTool
	{
		...
		private Action<int, string, int, string> _onChange; 

		public static void OpenAutoFillTool( Action<int, string, int, string>  onChange )
		{
			if (_instance != null)
			...
			switch (curLevel)
			{
				...
				case SettingsType.ProjectID:
					...
					_requester.RequestProjectIDs(_currentToken,
						merchantID: int.Parse(curNode.value),
						onSuccess: containers =>
						{
							if (TryRepackContainers(containers,curNode,out Node[] childNodes))
								foreach (var child in childNodes)
				...
				case SettingsType.LoginID:
					...
					_requester.RequestLoginIDs(_currentToken,
						merchantID: int.Parse(curNode.parent.value),
						projectID: int.Parse(curNode.value),
						onSuccess: containers =>
						{
							if (TryRepackContainers(containers,curNode,out Node[] childNodes))
								foreach (var child in childNodes)
				...
				case SettingsType.RedirectUrl:
					...
					TryAssignRedirectUrls(OAuthID: int.Parse(curNode.value), curNode, out Node[] _);
					finalCallback?.Invoke();
					break;
				default:
		...
		private void GoToPA(int[] selection, GotoType go)
		{
			...
			switch (go)
			{
				case GotoType.Login:
					{
						var url = new UrlBuilder(
								loginID != null
									? $"https://publisher.xsolla.com/{merchantID}/projects/{projectID}/login/{loginID}/navigation"
									: $"https://publisher.xsolla.com/{merchantID}/projects/{projectID}/login"
							)
							.Build();
				...
				case GotoType.Items:
					{
						var url = new UrlBuilder(
								$"https://publisher.xsolla.com/{merchantID}/projects/{projectID}/storefront/virtual-items")
							.Build();
						Application.OpenURL(url);
					}
					break;
