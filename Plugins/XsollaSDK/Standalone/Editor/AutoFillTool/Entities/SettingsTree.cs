...
namespace Xsolla.Core.Editor.AutoFillSettings
{
	internal class SettingsTree
	{
		...
		{
			public readonly string/*dynamic*/ value;
			public readonly string  valueView;
			public readonly TreeNode parent;
			public TreeNode[] children;

			public TreeNode(string value, string valueView, TreeNode parent)
			{
				this.value = value;
		...
		public string[] GetValues(int[] selection)
		{
			var result = new string[selection.Length];
			var curNode = _root;
			for (int i = 0; i < selection.Length; i++)
			{
				var children = curNode.children;
